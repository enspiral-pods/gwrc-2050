...
class TwentyFiftyServer < Sinatra::Base
  ...
  get '/calculator-bot-check/:region/:id' do |region, id|
    ...
    if botAgentStrings.any?{|bot| request.user_agent.include?(bot)}
      ...
      "<!DOCTYPE html><html><head><meta property=\"og:title\" content=\"Wellington Region 2050 Emissions Calculator\"><meta property=\"og:image\" content=\"https://og-image.2050calculator.nz/#{(percentage.abs * 100).round}?increase=#{percentage < 0}
